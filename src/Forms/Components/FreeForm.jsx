...
import { storage , freeForm } from '../config/firebase';
import { v4 } from 'uuid';
...
const FreeForm = () => {
  ...
  useEffect(() => {
    if(localStorage.getItem('free-form') === '1'){
        setSection(5);
    }
    else{    
        {(section === 5) && 
            <div>
                {localStorage.setItem('free-form', '1')}
            </div>
        }
    }
  },[section])  
  ...
  const handleStartupDetails = async (e) => {
    ...
    const imageRef = ref(storage, `freeFormImages/${photo.name+v4()}`);

    try {
      await uploadBytes(imageRef, photo); 
      const url = await getDownloadURL(imageRef);

      const docRef = await addDoc(freeForm, {
        name: name,
        email: email,
        phone: phone,
