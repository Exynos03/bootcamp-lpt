...
import { storage , scholarshipForm } from '../config/firebase';
import { v4 } from 'uuid';
...
const ScholarshipForm = () => {
    ...
    const parentOccupationOptions = [
        ...
        { value: 'Blue colour Job', label: 'Blue colour Job' },
    ...
    useEffect(() => {
      if(localStorage.getItem('scholarship-form') === '1'){
          setSection(6);
      }
      else{    
          {(section === 6) && 
              <div>
                  {localStorage.setItem('scholarship-form', '1')}
              </div>
          }
      }
    },[section])  
    ...
    const handleStartupDetails = async (e) => {
      ...
      const imageRef = ref(storage, `scholarshipFormImages/${photo.name+v4()}`);

      try {
        await uploadBytes(imageRef, photo); 
        const url = await getDownloadURL(imageRef);
  
        const docRef = await addDoc(scholarshipForm, {
          name: name,
          email: email,
          phone: phone,
    ...
    ?
      <>
        ...
        <div className='progress-bar'>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Fill} alt='Fill-img' className='bar-2'/>
            <img src={Empty} alt='Empty-img' className='bar-2'/>
        </div>
    ...
    <>
      ...
      <h1 className='form-head-1'>Hi {name.split(" ")[0]}</h1>
      <h1 className='form-head'>Please help us with some more details</h1>
      <form onSubmit={ (e) => handleScholarshipDetails(e)}>
            <label htmlFor="parent-occupation">Parents Occupation
                  <select
                  ...
                  >
                    ...
                    {parentOccupationOptions.map((option) => (
                ...
                <p className='txt-btm'>What does your parent do for work?</p>
            </label>
            <label htmlFor="family-income">Family Income
                  <select
                    id="family-income"
                    className='inpt-select'
                ...
                <p className='txt-btm'>What's your family's income?</p>
            </label>
            <label htmlFor="reason">Reason for scholarship
                  <div className='div'>
                      <textarea 
                          className='inpt-5'
                          type="text"
                  ...
                  <p className='txt-btm'>Why do you need a scholarship? </p>
              </label> 
              {
                (parentOccupation && familyIncome && scholarshipReason)
              ? 
                <button 
                      type="submit"
                      style={{backgroundColor:'#535353'}}
    ...
    ?
      ...
      <form onSubmit={ (e) => handlePersonalDetails(e)}>
              <label >Date of Birth
                  ...
                  {show === 1 && <Calendar onChange={dateSetter} value={selectedDate} maxDate={new Date()} />}
              </label>
